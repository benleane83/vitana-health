...
import { DuckDbPocRepository, digestHealthStoreData } from "../storage/duckdbPocRepository.js";
...
beforeEach(() => {
  root = initializePocRoot(mkdtempSync(join(tmpdir(), "lfa-duckdb-repository-test-")));
});
...
describe("DuckDbPocRepository fidelity", () => {
  it("refuses to create an empty replacement when opening a missing database", async () => {
    ...
    await expect(DuckDbPocRepository.open(root, databasePath, key)).rejects.toThrow("refuses to create");
  ...
  it.skipIf(!httpfsExtensionPath)("hydrates, closes, and reopens every v2 collection with exact content and order", async () => {
    ...
    const hydrated = await DuckDbPocRepository.hydrate(root, databasePath, key, fixture, options);
    try {
    ...
    await expect(DuckDbPocRepository.open(
      root,
      databasePath,
    ...
    const reopened = await DuckDbPocRepository.open(root, databasePath, key, options);
    let exported;
    try {
    ...
    await expect((async () => {
      let corruptRepository: DuckDbPocRepository | undefined;
      try {
        corruptRepository = await DuckDbPocRepository.open(root, corruptPath, key, options);
  ...
  it.skipIf(!httpfsExtensionPath)("updates a profile transactionally and preserves it across reopen", async () => {
    ...
    const fixture = createPocHealthStoreFixture();
    const options = { httpfsExtensionPath };
    const repository = await DuckDbPocRepository.hydrate(root, databasePath, key, fixture, options);
  ...
  it.skipIf(!httpfsExtensionPath)("provides slim transactional observation mutation primitives without snapshot responses", async () => {
    ...
    const fixture = createPocHealthStoreFixture();
    const repository = await DuckDbPocRepository.hydrate(root, databasePath, key, fixture, { httpfsExtensionPath });
  ...
  it.skipIf(!httpfsExtensionPath)("isolates profiles across concurrently open encrypted databases", async () => {
    ...
    const profileA = createPocHealthStoreFixture();
    const profileB = createPocHealthStoreFixture();
    ...
    const [repositoryA, repositoryB] = await Promise.all([
      DuckDbPocRepository.hydrate(root, profileAPath, profileAKey, profileA, options),
      DuckDbPocRepository.hydrate(root, profileBPath, profileBKey, profileB, options)
    ]);
    try {
      await Promise.all([
    ...
    await expect(DuckDbPocRepository.open(root, profileAPath, profileBKey, options)).rejects.toThrow();
    await expect(DuckDbPocRepository.open(root, profileBPath, profileAKey, options)).rejects.toThrow();
  }, 30_000);
  ...
  it.skipIf(!httpfsExtensionPath)("never promotes a database when hydration is terminated before atomic rename", async () => {
    ...
    expect(existsSync(databasePath)).toBe(false);
    const repository = await DuckDbPocRepository.hydrate(
      root,
      databasePath,
      key,
      createPocHealthStoreFixture(),
      { httpfsExtensionPath }
    );
    await repository.close();
  ...
  it.skipIf(!httpfsExtensionPath)("rolls back a delete when the child process is terminated before commit", async () => {
    ...
    const recovered = await DuckDbPocRepository.open(root, databasePath, key, { httpfsExtensionPath });
    try {
  ...
  it.skipIf(!httpfsExtensionPath)("upgrades a version-1 encrypted schema transactionally and idempotently on open", async () => {
    ...
    const options = { httpfsExtensionPath };
    await createPocSchema(root, databasePath, key, options, 1);

    const upgraded = await DuckDbPocRepository.open(root, databasePath, key, options);
    try {
  ...
  it.skipIf(!httpfsExtensionPath)("rejects missing, malformed, and future schema metadata without changing database bytes", async () => {
    ...
    const missingHandle = await openEncryptedPocDatabase(root, missingPath, key, options);
    await execSql(missingHandle.connection, "CREATE TABLE unrelated (id INTEGER);");
    await closeEncryptedPocDatabase(missingHandle);
    const missingHash = hashFile(missingPath);
    await expect(DuckDbPocRepository.open(root, missingPath, key, options)).rejects.toThrow("metadata is missing");
    ...
    await createPocSchema(root, malformedPath, key, options, 1);
    const malformedHandle = await openEncryptedPocDatabase(root, malformedPath, key, options);
    await execSql(malformedHandle.connection, "DELETE FROM poc_metadata;");
    await execSql(malformedHandle.connection, "CHECKPOINT;");
    await closeEncryptedPocDatabase(malformedHandle);
    const malformedHash = hashFile(malformedPath);
    await expect(DuckDbPocRepository.open(root, malformedPath, key, options)).rejects.toThrow("history is malformed");
    ...
    await createPocSchema(root, futurePath, key, options, 1);
    const futureHandle = await openEncryptedPocDatabase(root, futurePath, key, options);
    ...
    await closeEncryptedPocDatabase(futureHandle);
    const futureHash = hashFile(futurePath);
    await expect(DuckDbPocRepository.open(root, futurePath, key, options)).rejects.toThrow("newer than supported");
...
function runCrashWorker(mode: "hydrate" | "delete", databasePath: string): Promise<ChildProcess> {
  return new Promise((resolvePromise, reject) => {
    const workerPath = resolve(process.cwd(), "src", "poc", "duckdbCrashWorker.ts");
