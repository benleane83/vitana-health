...
import { initializePocRoot, type DuckDbPocOptions } from "./poc/duckdbPoc.js";
...
export class ProfileStoreManager {
  ...
  async activateDuckDb(options: DuckDbActivationOptions): Promise<void> {
    ...
    const root = initializePocRoot(options.root ?? duckdbStorageRoot());
    const existingManifest = loadStorageBackendManifest();
    const duckdbOptions: DuckDbPocOptions = {
    ...
    try {
      if (existingManifest) {
        assertManifestProfiles(existingManifest, this.profiles);
        ...
        this.commitDuckDbActivation(existingManifest, opened);
      ...
      this.commitDuckDbActivation(manifest, opened);
  ...
  createProfile(displayName: string): ProfileListEntry {
    if (this.backend === "duckdb") {
      throw new Error("Profile creation is temporarily unavailable during the DuckDB pilot.");
    }
  ...
  deleteProfile(profileId: string): { activeProfileId: string } {
    if (this.backend === "duckdb") {
      throw new Error("Profile deletion is temporarily unavailable during the DuckDB pilot.");
    }
    const normalizedId = normalizeProfileId(profileId);
