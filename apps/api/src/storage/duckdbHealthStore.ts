...
import {
  DuckDbPocRepository,
  type PocImport
} from "./duckdbPocRepository.js";
import type { DuckDbPocOptions } from "../poc/duckdbPoc.js";
...
export class DuckDbHealthStore {
  ...
  private constructor(
    private readonly repository: DuckDbPocRepository,
  ...
  static async open(options: DuckDbHealthStoreOptions): Promise<DuckDbHealthStore> {
    const repository = await DuckDbPocRepository.open(
      options.root,
      options.databasePath,
      deriveProfileDatabaseKey(options.passphrase, options.profileId),
      options.duckdb
  ...
  mergeImport(parsed: PocImport): Promise<HealthStoreData> {
    return this.enqueueMutation(async () => {
