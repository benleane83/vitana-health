...
In Standalone mode, the Android app stores health data in a local SQLCipher-encrypted SQLite database. Its database key is kept in Android secure storage and is restricted to the device. In Connected mode, the paired desktop app stores imported health data in the assigned profile's local AES-256-GCM encrypted DuckDB database; Dashboard and Track responses are held only in memory on the phone. Connection metadata is stored on the phone, and the pairing token and device identifier use Android secure storage. Report images and OCR drafts are cleared after commit, cancellation, disconnect, or app backgrounding.

There is no automatic health-data expiration or eviction. In Standalone mode, data remains in the encrypted mobile database until you delete observations or reset local data. In Connected mode, data remains until you delete individual observations, delete observations by type, or delete the profile in the desktop app; deleting a profile removes its local encrypted database. Disconnecting the companion deletes its local connection record and pairing token, and revoking a companion in the desktop app prevents its token from further access. Your Health Connect source data remains subject to the controls of Health Connect and its originating apps.
...
We protect companion transfers with HTTPS, certificate pinning, pairing approval, and revocable companion tokens. At rest, the desktop app encrypts profile databases with AES-256-GCM, while Standalone mode uses a SQLCipher-encrypted mobile database with its key in Android secure storage. The Android companion also uses secure storage for its token and device identifier. No security control can guarantee protection against every risk, especially on an unlocked or compromised device.
