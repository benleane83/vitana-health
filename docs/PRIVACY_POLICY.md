...
The companion also processes a random device identifier, pairing and connection metadata (the local server URL, public-key hash, pairing token, selected profile ID, selections, sync cursor, and last-sync time), and camera access for QR pairing. It does not retain camera images.

## How we use and transfer information

The companion uses selected Health Connect records to import data to the profile you choose on your paired Local Fitness Advisor desktop API. The API uses the data for local wellness analytics, deduplicated sync, and optional clinician-report exports. Health Connect data is not used for advertising, sold, or used for eligibility decisions.
...
The paired desktop app stores imported health data in the selected profile's local AES-256-GCM encrypted DuckDB database. Connection metadata is stored on the phone; the pairing token and device identifier use Android secure storage.
