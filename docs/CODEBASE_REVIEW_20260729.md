...
- **Non-atomic two-file writes.** `createProfile` ([L128](apps/api/src/storage/profileStoreManager.ts#L128)) writes the registry then the manifest with hand-rolled compensation; the delete path repeats it. A crash between the two leaves them disagreeing, and `loadStorageBackendManifest` throws `"Storage backend manifest is invalid."` — a hard startup failure. Write one combined document atomically (temp + fsync + rename).
...
- **`measurementDetails` returns an unbounded result when `limit` is undefined** ([duckdbProjections.ts:631-648](apps/api/src/storage/duckdbProjections.ts#L631)) — inconsistent with the disciplined clamping everywhere else in the same file (`min(max(limit,1),100)`, `maxAnalyticalRows`, `maxDailyChartBuckets = 366`).
- **`appBootstrap`/`storageCounts` run a 7-subquery `COUNT(*)` block** ([duckdbProjections.ts:129-136](apps/api/src/storage/duckdbProjections.ts#L129), [L798-806](apps/api/src/storage/duckdbProjections.ts#L798)) on every bootstrap and every import. Cache and invalidate on write.
...
- **Native timeouts do not cancel the request.** `pinnedFetch` races a `setTimeout`; the OkHttp call has no cancellation hook. A JS timeout firing before the Kotlin `callTimeout` produces a retry while the original upload is still in flight — the PC can receive the same chunk twice, and correctness rests entirely on undocumented server-side dedupe.
...
- **`setState` after `await` with no mount guard** at [useProfileLifecycle.tsx:51-64](apps/web/src/features/profiles/useProfileLifecycle.tsx#L51) and [SettingsPage.tsx:91](apps/web/src/pages/SettingsPage.tsx#L91) — the correct `cancelled` pattern is already used two lines away.
...
- **`packages/shared/src/index.ts` omits the parser modules**, so consumers use deep subpath imports that will break if `exports` restrictions are ever added.
...
1. Gate the release workflow on `validate:fast`; publish as draft (P1-1).
2. Ungate the CI jobs on push; move storage validation to `windows-2022` with the prepare step; make missing-extension a failure, not a skip (P1-16).
3. **Collapse to a clean v1 schema baseline** — indexes, foreign keys, `TIMESTAMPTZ`, canonical units at ingest, `raw_content` moved out, views out of migrations, retention defined (P1-3, P1-4, P1-9). Migrate your test profiles by hand. This is the single highest-leverage change on the list.
4. Pre-update backup on desktop, pre-migration backup on mobile, and non-fatal handling of a newer-than-supported schema on both (P1-2, P1-8).
5. Bump the Android `version` policy so `runtimeVersion` isolates schema/native generations (P1-8). Add the two undeclared dependencies (P1-18).

**Before testers accumulate data**

6. Replace the whole-store replica snapshot with the explicit-changes overload (P1-5); fix the mobile migration N+1 and the two scan-per-write patterns (P1-6).
7. Rewrite Health Connect sync as a stream with a per-category cursor, per-chunk checkpoints, cancellation, and a module-scope single-flight guard (P1-7). Rename the persisted keys to platform-neutral names in the same change.
8. Give reads their own connection (P1-10).
9. Add `AbortSignal` to the transport and a request-generation token to profile switching (P1-13) — this is a privacy defect, not just a race.
10. Add error boundaries to both UIs (P1-14). Cheapest possible insurance.
11. Real response schemas with `z.infer`-derived types; protocol version negotiation for replica sync before any client is published (P1-11).
12. Route backup restore through the migrating parser and split the error codes (P1-12).
13. The four small performance fixes: `Map`-based chart dedupe, `reduce` instead of spread, memoize the summary sort, hoist the regex (P1-15).
14. Android key-loss diagnosis, rekey path, and removing the string-matched destructive recovery (P1-19).
15. Desktop lifecycle: migration inside the single-instance lock, shutdown timeout, storage cleanup on startup failure, process-level rejection handlers (P1-17).

**Then, at a comfortable pace**

16. Define the `HealthSourceProvider` and pinned-HTTP TypeScript contracts now; implement iOS later (P2).
