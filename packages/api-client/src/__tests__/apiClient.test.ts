  ...
  it("supports paginated care queries and mutation methods", async () => {
    ...
    const transport = async (request: ApiTransportRequest) => {
      ...
      if (request.path === "/api/care/items/care-1/complete" && request.method === "POST") {
        return response({
          careItem: { id: "care-1", title: "Book check-in", kind: "visit", priority: "normal", status: "completed", completedAt: "2026-01-02T00:00:00.000Z", completedHealthEventId: "event-2" },
      ...
      return response({ careItem: { id: "care-1", title: "Book check-in", kind: "visit", priority: "normal", status: "open" }, counts: { imports: 0, observations: 0, samples: 0, activities: 0, healthEvents: 0, careItems: 1 } });
    };
    const client = createApiClient(transport);
    ...
    await client.listCareItems({ limit: 10, status: "open", kind: "visit" });
    await client.createCareItem({ title: "Book check-in", kind: "visit", priority: "normal", status: "open" });
    ...
    expect(seen[2]).toMatchObject({ method: "GET", path: "/api/care/items?limit=10&kind=visit&status=open" });
