...
export function parseBloodTestScanText(fileName: string, sourceText: string, importedAt = new Date().toISOString()): BloodTestDraft {
  ...
  const sourceChecksum = checksum(normalizedText || fileName);
  const diagnostics: string[] = [];
  const reportDate = readBloodTestReportDate(normalizedText) ?? readBodyCompositionDate(normalizedText) ?? readDateFromFileName(fileName);
  const rows = new Map<string, BodyCompositionDraftRow>();
...
function parseKnownBloodTestLine(line: string): {
  ...
  const candidates = defaultMeasurementTypes
    ...
    .flatMap((measurementType) => [measurementType.display, ...measurementType.aliases]
      .map((label) => ({ label, measurementType })))
  ...
  for (const candidate of candidates) {
    ...
    return {
      ...
      value,
      unit: normalizeBloodTestUnit(result?.[2], candidate.measurementType.canonicalUnit)
    };
  }
  return undefined;
}
...
function normalizeBloodTestUnit(unit: string | undefined, canonicalUnit: string): string {
  ...
  if (normalized === normalizedCanonical || (normalized.startsWith("u") && `µ${normalized.slice(1)}` === normalizedCanonical)) {
